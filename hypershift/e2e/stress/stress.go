...
var (
	RequestsTotal = prometheus.NewCounter(
		prometheus.CounterOpts{
			Name: "etcd_e2e_stress_requests_total",
			Help: "Number of requests sent to etcd",
		},
	)
	FailedRequestsTotal = prometheus.NewCounter(
		prometheus.CounterOpts{
			Name: "etcd_e2e_stress_failed_requests_total",
...
func (s *Stressor) Start(ctx context.Context, c *clientv3.Client) {
	s.client = c
...
func (s *Stressor) run(ctx context.Context) {
	defer s.wg.Done()
...
func (s *Stressor) choose() func(context.Context) (error, int64) {
	r := rand.Float32()
	switch {
	case r < 0.15:
		return s.put
...
func (s *Stressor) put(ctx context.Context) (error, int64) {
	_, err := s.client.Put(
		ctx,
...
func (s *Stressor) putTxn(ctx context.Context) (error, int64) {
...
func (s *Stressor) putBig(ctx context.Context) (error, int64) {
	_, err := s.client.Put(
		ctx,
		fmt.Sprintf("foo%016x", rand.Intn(stresserKeySuffixRange)),
...
func (s *Stressor) getSingle(ctx context.Context) (error, int64) {
	_, err := s.client.Get(ctx, fmt.Sprintf("foo%016x", rand.Intn(stresserKeySuffixRange)))
	return err, 0
}

func (s *Stressor) getRange(ctx context.Context) (error, int64) {
...
func (s *Stressor) deleteSingle(ctx context.Context) (error, int64) {
	_, err := s.client.Delete(ctx, fmt.Sprintf("foo%016x", rand.Intn(stresserKeySuffixRange)))
	return err, 1
}

func (s *Stressor) deleteRange(ctx context.Context) (error, int64) {
	start := rand.Intn(stresserKeySuffixRange)
	end := start + 500
